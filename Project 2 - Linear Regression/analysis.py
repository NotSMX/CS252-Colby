...
class Analysis:
    ...
    def var(self, headers, rows=[]):
        ...
        arr = self.data.select_data(headers,rows)
        difference = np.subtract(arr, mean)
        summation = np.abs((difference)**2)
        sum = np.sum(summation, axis = 0)
        length = np.size(arr, axis = 0) - 1
        return sum/length
    ...
    def scatter(self, ind_var, dep_var, title):
        ...
        '''
        arr = self.data.select_data(headers = [ind_var, dep_var])
        x = arr[:,0]
        y = arr[:,1]
        plt.scatter(x,y)
        plt.xlabel(ind_var)
    ...
    def pair_plot(self, data_vars, fig_sz=(12, 12), title=''):
        ...
        fig, axes = plt.subplots(len(data_vars), len(data_vars), sharex = 'col', sharey = 'row')
        plt.subplots_adjust(top = 5, right = 5)
        plt.title(title, fontsize = 20)    
        for i, ind_var in enumerate(data_vars) :
            for j, dep_var in enumerate(data_vars) :
                axs = axes[i][j]
                if i == len(data_vars) -1:
                    axs.set_xlabel(data_vars[j])
                if j == 0 :
                    axs.set_ylabel(data_vars[i])
                axs.scatter(self.data.select_data(headers = [dep_var]), self.data.select_data(headers = [ind_var]), c = "c")
        return fig, axes
