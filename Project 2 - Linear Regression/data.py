...
class Data:
    ...
    def read(self, filepath):
        ...
        with open(filepath) as file:
            # inspect the first and second row for headers and data types
            ...
            if (firstType != "numeric" and firstType != "categorical" and firstType != "string"):
            ...
            count = 0
            for line in file:
                # line will be defined as the next line (curr line) as a string. Calling readline when doing this way not needed.
                # checking if the line isn't blank
                if line.strip() :
                    # add to a list
                    temp = []
                    ...
                    for j in range(0, len(splitline)) :
                        if j not in columns2ignore:
                            ...
                            # print(splitline)
                            # print(field.strip())
                            # print(splitline.index(field))
                            # print(self.headers)
                            # print(self.cats2levels)
                            # print(j)
                            # print(columns2ignore)
                            # print(self.header2col)

                            if j in indices: # numeric cols indices
                                # check blank space
                                if field == '' :
                                    temp.append(np.nan)
                                else:
                                ...
                                # remove duplicates
                                # if currLevel not in cat2levels[key]: -> append
                                # regardless, look up int index from there
