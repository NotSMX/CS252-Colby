...
class PCA:
    ...
    def covariance_matrix(self, data):
        ...
        '''
        A_centered = data - np.mean(data, axis = 0)
        matrix = (1/(data.shape[0] - 1)) * (A_centered.T @ A_centered)
        return matrix
    ...
    def fit(self, vars, normalize_dataset=False):
        ...
        cov_matrix = np.real(self.covariance_matrix(self.A))
        e_vals, e_vecs = np.linalg.eig(np.real(cov_matrix))
        sorted_indices = np.argsort(e_vals)[::-1]
        self.e_vals = np.real(e_vals[sorted_indices])
        self.e_vecs = np.real(e_vecs[:, sorted_indices])
    ...
    def elbow_plot(self, num_pcs_to_keep=None):
        ...
        '''
        sort_eig = np.argsort(self.e_vals)[::-1] 
        
        if (num_pcs_to_keep is None):
            plt.plot(sort_eig + 1, self.cum_var, linestyle = '-', marker = 'o')
            plt.title("PCs vs. Cumulative Proportion Variance")
            plt.xlabel("PCS")
            plt.ylabel("Cumulative Proportion")
        else:
            sort_eig = np.argsort(self.e_vals[:num_pcs_to_keep])[::-1]
            plt.plot(sort_eig + 1, self.cum_var[:num_pcs_to_keep],  linestyle = '-', marker = 'o')
            plt.title("PCs vs. Cumulative Proportion Variance")
            plt.xlabel("PCS")
            plt.ylabel("Cumulative Proportion")
        pass
    ...
    def pca_project(self, pcs_to_keep):
        ...
        '''
        P = self.e_vecs[:, pcs_to_keep]
        A_centered = self.A - np.mean(self.A, axis = 0)
    ...
    def pca_then_project_back(self, top_k):
        ...
        if self.normalized == True:
            projected_data = self.range * ((self.pca_project(list) @ P.T) + np.mean(self.A, axis = 0)) + self.min
        else:
            projected_data = self.pca_project(list) @ P.T + np.mean(self.A, axis = 0)
